...
ui <- page_sidebar(
  ...
  layout_columns(
    ...
    value_box(title= tags$div("Total Number of Species Caught",
                              style = "font-size: 20px"),
              value = textOutput("species_count")),
    value_box(title= tags$div("Total Number of Fishing Entities",
                              style = "font-size: 20px"),
              value = textOutput("fe_count")),
    
    card(card_header("Fisheries Catch in United Kingdom Waters",
                     style="font-size: 20px"),
...
server <- function(input, output, session){
  ...
  output$timeseries <- renderPlotly({
    ...
    time_series_plot <- grouped |> 
      ...
      geom_point(aes(text = paste(!!sym(input$breakdown), "<br>",
                                  "Year:", year, "<br>",
                                  "Catch:", round(total, 2),
                                  "Thousand Tonnes")),
                 alpha=0, size = 0) +
